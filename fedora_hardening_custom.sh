...
# 
# Domain: ccdcteam.com
# Authentication: SSSD (Active Directory)
# 
# SCORED SERVICES:
#   SMTP (25) - Postfix
#   POP3 (110) - Dovecot
...
########################################
log "Verifying SSSD configuration"

if systemctl is-active --quiet sssd 2>/dev/null; then
  log "✓ SSSD is running"
  if [[ -f /etc/sssd/sssd.conf ]]; then
    DOMAIN=$(grep "^domains" /etc/sssd/sssd.conf 2>/dev/null | cut -d'=' -f2 | tr -d ' ')
    log "SSSD Domain: $DOMAIN"
  fi
else
  warn "SSSD is not running! Expected for AD authentication."
  warn "This script assumes SSSD is configured for ccdcteam.com"
fi
...
echo ""
echo "┌─────────────────────────────────────────────────────────────┐"
echo "│  ⚠️  ACTIVE DIRECTORY AUTHENTICATION DETECTED              │"
echo "├─────────────────────────────────────────────────────────────┤"
echo "│  This server uses SSSD for ccdcteam.com domain.            │"
echo "│                                                             │"
...
# Initialize database
if [[ -f "$ROUNDCUBE_DIR/SQL/mysql.initial.sql" ]]; then
...
echo "  4. Login with AD credentials (user@ccdcteam.com)"
echo "  5. Change AD passwords on Windows server!"
echo "  6. Monitor scoring dashboard"
echo ""
warn "REMEMBER: Mail user passwords must be changed on AD server!"
echo ""
